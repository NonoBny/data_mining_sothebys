...
item_columns = [Utilility.Column('collection_id', 'int(16)', ['NOT NULL']),
                ...
                Utilility.Column('title', 'varchar(65535)', ['COLLATE utf8_bin']),
                Utilility.Column('number_of_items', 'int(16)', []),
...
def load_collection_table(collections):
    map(lambda c_id, c: insert_into_collections_table(c, c_id), range(len(collections), collections))
    return
...
def load_items_table(items, collection_id):
    map(lambda item: load_items_table(item, collection_id), items)
